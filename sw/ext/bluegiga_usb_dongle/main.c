...
void ble_evt_connection_status(const struct ble_msg_connection_status_evt_t *msg)
{
    ...
    // Connection request completed
        ...
        printf("Connected, nr: %d, connection interval: %d = %fms\n",msg->connection, msg->conn_interval, msg->conn_interval*1.25);
    	...
    	if(state==state_listening_measurements)
    	{
			...
			while(ac_id[device]!=-1 && device < 8){
				if(connected[device] && sock[device]){
					bytes_recv = recvfrom(sock[device],recv_data,1024,MSG_DONTWAIT,(struct sockaddr *)&rec_addr[device],(socklen_t *)&sin_size);
					if( bytes_recv > 0){
...
{ 
    ...
    if (action == action_scan) {
    	...
    	ble_cmd_gap_set_adv_parameters( 0x200, 0x200, 0x07);		// advertise interval scales 625us, min, max, channels (0x07 = 3, 0x03 = 2, 0x04 = 1)
    }
